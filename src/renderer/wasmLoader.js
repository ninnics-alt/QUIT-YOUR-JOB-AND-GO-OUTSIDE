// Simple WASM loader wrapper for ebur128 if provided in src/wasm/ebur128.wasm
// Exposes window.EBUR128 when available. Falls back to null if not.
(async function(){
  window.EBUR128 = null;
  try{
    ...
    // Note: exact export names depend on how you compile the library/wrapper.
    // This loader assumes a minimal wrapper that exports functions named:
    //  - create_ebur128(sampleRate, channels) -> pointer
    //  - destroy_ebur128(ptr)
    //  - ebur128_add_frames(ptr, ptrSamples, frames)
    // WASM loader wrapper for ebur128.
    // Behavior:
    // 1) Try to load Emscripten glue at `wasm/ebur128.js` and use `Module.cwrap`.
